...
bool ClodaService::itsNeighbors(std::pair<int, int> posOne, std::pair<int, int> posTwo, std::vector<std::pair<int, int>> &visited) {
    //return  visited.size() != 1 && (visited.end() - 1)->first != posTwo.first && (visited.end() - 1)->second != posTwo.second && ((posOne.first == posTwo.first + 1 && posOne.second == posTwo.second) || (posOne.first == posTwo.first - 1 && posOne.second == posTwo.second) || (posOne.first == posTwo.first && posOne.second == posTwo.second + 1) || (posOne.first == posTwo.first && posOne.second == posTwo.second - 1));
...
bool ClodaService::solveGraph(std::pair<int, int> posBegin, std::pair<int, int> posEnd, std::vector<std::pair<int, int>> &visited, std::vector<std::pair<int, int>> &path) {
    ...
    for(int row = 0; row < this->dimensions.first; row++) {
        for(int col = 0; col < this->dimensions.second; col++) {
            if(!this->posWasVisited({row, col}, visited) && this->graph[row][col] != 1000 && this->itsNeighbors(posBegin, {row, col}, visited)) {
